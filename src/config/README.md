...
博客正文可以用 `# 一级标题` 明确文章标题，并应把它放在正文开头；存在 H1 时，文章页、列表、搜索与 SEO 都优先使用它。没有 H1 时则回退到 frontmatter 的 `title`，兼容旧文章和外部导入内容。Markdown 渲染插件会把正文 H1 交给文章页头排版，因此最终页面不会产生重复标题。后续章节从 `##` 开始；`npm run content:check` 会拒绝错位或重复的一级标题。
