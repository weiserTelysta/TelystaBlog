...
- 为正文图片接入按需 PhotoSwipe，看图器支持鼠标、键盘与手机缩放，保留正文视觉和作者已有链接。关闭恢复阅读位置与焦点，减少动态效果时立即切换。
- 浏览器回归发现并修复 Tab 越过末尾控件的问题；补齐打开动画期间关闭、页面切换清理与模块加载失败回退。实现、范围和验证见 [开发记录](article-image-viewer-2026-09-24.md)。
- 110 项单元测试、52 项公开页面浏览器回归、87 份内容校验和 31 页生产构建通过。提交同时保留作者自行润色的新委托日文正文，包括「死のやさしさと生の希望」与发型交由画师发挥的补充。
...
- 后续调整去掉新服饰的实用、朴素风格要求；性格锚点改为「人を包み込むような温かさ」，保留包容温暖的气质，服饰设计交由画师自由发挥。
- 公开后验证：87 个内容文档零错误／警告，生产构建 31 页成功，新旧委托构建产物均使用新 WebP 地址，源码不再引用原 PNG。经作者确认公开文章并提交与推送。
- 首次发布 `d522400` 的 [云端构建与部署](https://github.com/weiserTelysta/TelystaBlog/actions/runs/35909246414) 成功，线上新委托返回 HTTP 200，收殓职责与 WebP 地址均已核对。
...
- 接入 Micromark WikiLink tokenizer，支持目标名／显示文字；内容检查和后台保存检查重复、缺失及草稿目标。后台预览保留隔离、禁用跳转。
- 目标改名／移动会使引用文章缓存失效；在隔离副本中验证保留缓存的构建和运行中的 dev 自动更新，无需修改引用正文。完整边界和检查结果见 [开发记录](wikilinks-2026-09-21.md)。

## 2026-09-21：角色虹彩、资源与发布收尾

- Series 分组仅显示英文分类名；分类卡片减少曝光面积，反射以角色配色为核心，后台同步预览。
- Telysta 中亚风格插画收录资源页，上传独立封面和高清 WebP，三种 CDN 文件逐一核对哈希；补齐三篇文章的空缺英文元数据与摘要。
- 核对全部公开页面和后台回归，修正硬编码图片数量与弹窗关闭等待的测试假设；完整结果见 [发布记录](category-resource-release-2026-09-21.md)。
- 完成 [单 Markdown 多语言研判](multilingual-markdown-assessment-2026-09-21.md)，未实现多语言功能。
- 修复原图尺寸补全与清单变更后的文章缓存失效，定点更新 devalue；103 项单元测试、41 项公开页面场景、6 项后台场景通过，依赖审计零漏洞。`2fddbd6` 推送后 Linux 构建和 Pages 部署成功，线上资源、系列、文章、歌谱与样式核验通过；详见发布记录中的 Actions 链接。

## 2026-09-21：分类光泽、Series 与观察日志歌谱

- 调研卡牌案例的分层反射，前后台共享八种预设，去掉同心圆和重复样式；新增响应式 WebP，区分构建优化、Cloudflare 缓存与 R2 分发。
- Series 索引统一英文所属题签与系列资料，改为留白分组；目录保留键盘焦点和手机布局。补齐用户新文章引用的 `rhaelysa-notes` 配置。
- 日志 02 加入《慈光歌》主旋律版，03 加入《耶稣基督是生命源》；修复简谱调号解析和歌词分组回退。保留作者现有改动及日志 02 重命名。
- 完整构建、测试、图片体积、谱面范围与部署状态见 [本轮验收记录](category-series-scores-2026-09-20.md)。
...
- 首次推送 `fdb8dbc` 的云端测试发现 Linux 新建路径遗漏大小写冲突，部署被测试阻止；补齐不存在文件的同级名称检查，并新增可在 Windows 上执行的 Linux 分支回归。根因与失败运行链接见 [云端检查记录](admin-ux-plan-2026-09-15.md#首次云端检查发现的跨平台问题)。
- 修复提交 `765db6d` 的 [云端构建与部署](https://github.com/weiserTelysta/TelystaBlog/actions/runs/34975006396) 均成功。部署后 7 个主要页面与英文题签核对通过，15 个脚本／样式与本地产物逐字节一致；本地最终完整检查为 99 项单元测试通过。详见 [部署验收](admin-ux-plan-2026-09-15.md#云端部署验收)。
