...
R2 中与主图同名的 PNG/JPG 原图会自动加入下载列表，不必重复写 `actions`。当前看图器只从解析后的下载项保留允许的图片格式，不展示普通 external／source 页面或 ZIP／PSD 下载；这些源数据仍可留存。网站过滤 PSD 不改变 R2 对象访问权限。
