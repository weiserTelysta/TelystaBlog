...
- 点图直接进入全屏看图器，按同一条目的 `gallery` 顺序左右切换。前后箭头位于图片两侧中部，下载与关闭位于图片底部右侧，标题位于下缘；单张不显示箭头，首尾不循环。鼠标离开图片或约 2.2 秒未操作后隐藏文字与操作阴影，键盘操作时恢复；触控不依赖 hover。
...
本次选择依据、执行步骤与待验收项见 [资源画廊重构计划与记录](archive/2026-09/resource-gallery-2026-09-05.md)。
